...
const RoleModel = mongoose.model('learnstoreRoles', new mongoose.Schema({
...
const UserModel = mongoose.model('lernstoreUsers', new mongoose.Schema({
	roles: [{ type: mongoose.Types.ObjectId, ref: 'role' }],
	schoolId: {
		type: mongoose.Types.ObjectId, ref: 'school', required: true, index: true,
	},
}, {
	timestamps: true,
}), 'users');

const SchoolModel = mongoose.model('lernstoreSchools', new mongoose.Schema({
	federalState: { type: mongoose.Types.ObjectId, ref: 'federalstate' },
}, {
	timestamps: true,
}), 'schools');

const FederalStateModel = mongoose.model('lernstoreFederalstates', new mongoose.Schema({
